...
- **Redirect en Next.js, no en API:** route handler `apps/web/src/app/c/[code]/route.ts` — resuelve vía API pública y hace `redirect()` 302 a `/?slug=<slug>&source=short` (o al dominio propio de la campaña si existe). Mantiene el flujo multidominio server-side existente.
- **Endpoint API público:** `GET /v1/public/short/{code}` → `{slug, domain}` o 404. Sin datos sensibles, cacheable.
- **QR client-side se mantiene** (patrón actual con librería `qrcode` en el editor): solo cambia el contenido codificado → `https://<dominio>/c/<short_code>?source=qr`, y se agrega botón de descarga PNG 1024px (`QRCode.toDataURL(url, {width: 1024})` → `<a download>`).
...
| Archivo | Cambio |
|---------|--------|
| `apps/web/src/app/c/[code]/route.ts` | route handler redirect 302 |
