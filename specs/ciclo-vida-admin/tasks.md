...
- [x] **T1** Agregar `LifecycleEventOut` schema en `campaign.py`:
  `id`, `stage`, `stage_index`, `notes`, `registered_at`, `registered_by` (R3, R4)

- [x] **T2** Agregar `LifecycleStageUpdate` schema: `stage: int` (ge=0, le=4),
  `notes: str | None` (max_length=500), `notify_org: bool = False` (R1, R2, R13)

- [x] **T3** Agregar `NotifySignersRequest` schema: `message: str` (min=1,
  max_length=1000) (R14, R15)

- [x] **T4** Extender serialización admin de campaña con `lifecycle_stage: int`
...
- [x] **T5** Agregar `platform_admin_emails: str = ""` en `config.py`; parsear
  como lista al usar (`[e.strip() for e in v.split(",") if e.strip()]`) (R12)

- [x] **T6** `send_lifecycle_admin_notification(campaign_title, org_name, old_stage,
...
- [x] **T7** `send_lifecycle_org_notification(to_email, campaign_title, new_stage,
  notes)` en `email_service.py`: fire-and-forget a `org.contact_email`;
  no-op si email nulo (R13)

- [x] **T8** `send_lifecycle_signer_notification(db, campaign_id, campaign_title,
...
- [x] **T9** Migración `014_consents_notify_updates.py`: agregar columna
...
- [x] **T10** `campaign_service.update_lifecycle_stage(db, campaign, new_stage, notes,
...
- [x] **T11** `PATCH /v1/campaigns/{id}/lifecycle`: valida campaña no archivada (409),
  llama al service, responde `{ lifecycle_stage, event, notifications_sent }` (R6, R7)

- [x] **T12** `POST /v1/campaigns/{id}/lifecycle/notify-signers`: valida campaña no
...
- [x] **T13** Agregar a `AdminCampaign` en `admin-campaigns-api.ts`:
  `lifecycle_stage: number` y `lifecycle_events: LifecycleEventOut[]`;
  definir tipo `LifecycleEventOut` (R10)

- [x] **T14** Función `advanceLifecycleStage(campaignId, stage, notes, notifyOrg)` →
  `PATCH /v1/campaigns/{id}/lifecycle` (R1, R2, R13)

- [x] **T15** Función `notifySigners(campaignId, message)` →
